# Manually Drive Your  car

The aim of this project is to document how to manually drive an  car vehicle using a joystick controller or a steering wheel. Out-of-the-box you can do this using a mouse with the vehicle's built-in user interface but it is a lot more fun to drive the car around with a joystick or steering wheel.

## History

The history of this project:
* this is a fork from https://github.com/lshw54/car_control
* which was a refactor from https://github.com/thu2004/car-vehicle-api 
* which was a refactor from https://github.com/ARCC-RACE/car-rc
...
* Updating libaries
* Updated reflect changes to  car APIs
...
There are lots of ways to learn about machine learning and robotics but  car is the most fun way to do it.

You will first need an  car vehicle which you can order from .com. There are two models: the standard  car that is 1/18 scale race car with a single camera; and an  car Evo model which includes the standard model and an extra kit with a second camera, lidar sensor, and a larger shell for the vehicle. You can get started with the standard model and you can add the sensor kit later once you are ready for building advance machine learning models on the car.

### Learning About Machine Learning

You can learn about machine learning at the  website.


### Learning About Robotics

https://github.com/car/car-launcher
https://..com/car/robotics-projects/

If you want to  car code is now open source and available on github.



## Pygame - Python for Video Games

[Pygame](https://www.pygame.org) is a set of Python modules designed for writing video games. This project uses Pygame's joystick library to translate input from compatible joysticks. The joystick input is translated to PUT requests to the car API. The  car cars exposes an API to control the vehicle using the Robot Operating System (ROS2). The Pygame website includes instructions for [getting started](https://www.pygame.org/wiki/GettingStarted) with Pygame.

To drive an  car car using this library, you first need to test if the joystick you want to use works with Pygame. If it does not work with Pygame then you can't use this library.
...
For Windows, a simple way to install Python3 and Git is with Chocolatey - the package manager for Windows - https://github.com/lshw54/car_control
...
* Checkout the repository ```git clone https://github.com/jacobcantwell/car_api.git```
* Change to the directory ```cd car_api```
...
* Add a **car_PASSWORD** value the car UI password.

```yaml
LOCAL_IP=[local-ip]
car_PASSWORD=[password]
```

## Controlling car

The scripts for controlling with different joysticks are similar except for how the joystick input from Pygame is translated to  car input.

### Controlling car With a generic PS4 joystick
...
### Controlling car With a PS4 joystick
...
The  car car includes a 640x480 colour camera and publishes a video stream to a ROS2 topic. You can view the video stream by opening the topic as seen in this python script below. Open a seperate terminal to run this script and you can control and view the  car car at the same time.
