...
def boxes_to_bev_polygons(boxes):
    """将3D框转换为BEV多边形[5,7](@ref)
    Args:
    ...
    for box in boxes:
        ...
        polygons.append(Polygon(translated))
    return polygons

def custom_boxes_overlap_bev(boxes_a, boxes_b):
    """替代iou3d_nms_cuda.boxes_overlap_bev_gpu"""
    poly_a = boxes_to_bev_polygons(boxes_a)
    poly_b = boxes_to_bev_polygons(boxes_b)
    
    overlaps = np.zeros((len(boxes_a), len(boxes_b)))
    for i in range(len(poly_a)):
        for j in range(len(poly_b)):
            if poly_a[i].intersects(poly_b[j]):
                overlaps[i][j] = poly_a[i].intersection(poly_b[j]).area
    return overlaps

def custom_boxes_iou_bev(boxes_a, boxes_b):
    """替代iou3d_nms_cuda.boxes_iou_bev_cpu"""
    poly_a = boxes_to_bev_polygons(boxes_a)
    poly_b = boxes_to_bev_polygons(boxes_b)
    ...
    for i in range(len(poly_a)):
        for j in range(len(poly_b)):
            intersection = poly_a[i].intersection(poly_b[j]).area
            union = poly_a[i].area + poly_b[j].area - intersection
...
class eval3d():
    ...
    def eval_iou(self, gt_boxes3d, boxes3d):
        ...
        gt_boxes3d_gpu = torch.tensor(gt_boxes3d, dtype=torch.float32)
        #boxes3d_gpu = torch.tensor(boxes3d, dtype=torch.float32).cuda()  # load to gpu
        boxes3d_gpu = torch.tensor(boxes3d, dtype=torch.float32)
...
def boxes_iou_bev_cpu(boxes_a, boxes_b):
    ...
    boxes_a_np = boxes_a.cpu().numpy() if isinstance(boxes_a, torch.Tensor) else boxes_a
    boxes_b_np = boxes_b.cpu().numpy() if isinstance(boxes_b, torch.Tensor) else boxes_b
    ans_iou = custom_boxes_iou_bev(boxes_a_np, boxes_b_np)
...
def boxes_iou3d_gpu(boxes_a, boxes_b):
    ...
    boxes_a_np = boxes_a.cpu().numpy() if isinstance(boxes_a, torch.Tensor) else boxes_a
    boxes_b_np = boxes_b.cpu().numpy() if isinstance(boxes_b, torch.Tensor) else boxes_b
    overlaps_bev = custom_boxes_overlap_bev(boxes_a_np, boxes_b_np)
