...
int main (int argc, char** argv)
{
    ...
    reader.read (package_path + "/dataset/point_cloud_data/point_cloud.pcd", *cloud); // cloud已经过滤波和降采样
    ...
    while (cloud->points.size () > 0.3 * nr_points) //***该循环不断地去除平面
    {
        ...
        pcl::io::savePCDFileASCII(package_path + "/dataset/point_cloud_data/"+s_plane.str(),*cloud_plane); //2022.2.23设置了保存路径
    ...
    for (std::vector<pcl::PointIndices>::const_iterator it = cluster_indices.begin (); it != cluster_indices.end (); ++it)
    {
        ...
        writer.write<pcl::PointXYZ> (package_path + "/dataset/point_cloud_data/"+ss.str (), *cloud_cluster); //*
        j++;
    }
    ...
    pcl::io::loadPCDFile(package_path + "/dataset/point_cloud_data/cloud_cluster_0.pcd",*view0);
    pcl::io::loadPCDFile(package_path + "/dataset/point_cloud_data/cloud_cluster_1.pcd",*view1);
    pcl::io::loadPCDFile(package_path + "/dataset/point_cloud_data/cloud_cluster_2.pcd",*view2);
    pcl::io::loadPCDFile(package_path + "/dataset/point_cloud_data/cloud_cluster_3.pcd",*view3);
    pcl::io::loadPCDFile(package_path + "/dataset/point_cloud_data/cloud_cluster_4.pcd",*view4);
    //读取两个平面
    pcl::io::loadPCDFile(package_path + "/dataset/point_cloud_data/cloud_plane_0.pcd",*view_plane1);
    pcl::io::loadPCDFile(package_path + "/dataset/point_cloud_data/cloud_plane_1.pcd",*view_plane2);
