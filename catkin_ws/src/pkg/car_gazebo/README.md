...
The  car repository contains the configuration and launch files to enable ROS Navigation Stack on  car and control the vehicle using teleop-twist-keyboard, along with the core components to integrate  car with ROS Navigation stack. For detailed information, see [Introduction to the ROS Navigation stack using  car Evo](https://github.com/car/car/blob/main/introduction-to-the-ros-navigation-stack-using-car-evo.md).

 car uses [Ackermann front-wheel steering](https://en.wikipedia.org/wiki/Ackermann_steering_geometry) to turn wheels on the inside and outside of a turn. This mean that the left and right front wheels generally turn at different angles. The **car_gazebo** package contains the **car_drive** gazebo plugin that has subscribes to the `/cmd_vel` topic and publishes the corresponding joint velocity and position commands to move the car in Gazebo environment. It also takes care of publishing the `/odom` messages required by the Nav2 packages.

## License

The source code is released under [Apache 2.0](https://..com/apache-2-0/).

## Plugin Details

### Example:

        <plugin name='gazebo_ros_car_drive' filename='libgazebo_ros_car_drive.so'>
...
* [Getting started with  car OpenSource](https://github.com/car/car-launcher/blob/main/getting-started.md)
* [Introduction to the ROS Navigation stack using  car Evo](https://github.com/car/car/blob/main/introduction-to-the-ros-navigation-stack-using-car-evo.md)
