...
def iris1_callback(msg):
    ...
    if start_follow:
        x = -(target_position.x - current_position.x - init_gap[0]) * 1
        y = -(target_position.y - current_position.y - init_gap[1]) * 1
    
    # 升高到一定高度时开始识别二维码                
    if (target_hight - current_position.z) < 2:
        start_follow = True   
    # 无人机保持指定高度飞行
    if (current_position.z - target_hight) > 0.2:
        #print("下降")
        z = -0.5
    elif (current_position.z - target_hight) < -0.2:
        #print("升高")
        z = 0.5
    else:
        z = 0
...
if __name__=="__main__":
    ...
    rospy.init_node('uav1_follow') #创建ROS节点
    ...
    # uav1起飞
    takeoff()
    # 订阅ugv0/1上的Odometry信息
    iris1_sub = rospy.Subscriber("/ugv0/odom", Odometry, iris1_callback)
    rospy.spin()
