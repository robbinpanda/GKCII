...
class VisualAssistant:
    ...
    def scene_output(self, frame: int, zs: ObsBundle, kf: Kalman):
        ...
        for x in kf.xpts:
            ...
            #print(c,type(tuple(c[0:2])),my_color,self.base_image.shape)
            cv2.circle(self.base_image, tuple(c[0:2].tolist()), 4, my_color, -1)
        # Sensor Data
        for p, s in zip(zs.projections, zs.sensors):
            ...
            cv2.circle(self.base_image, tuple(c[0:2].tolist()), 2, my_color, -1)
